"""Precision extension of the frozen fresh held-out replication.
...
RUN_MODE = "full"
    Runs the frozen second fresh 24-prompt precision-extension population. Each prompt is
...
SCRIPT_VERSION = "0.8.0-fresh-heldout-extension"
RESUME_PATCH = (
    "2026-09-11: precision extension; second fresh held-out batch, seeds, "
    "conditions, scaling, scoring, and analysis fixed before extension outcomes"
)
...
GENERATION_SEED_BASE = 26093000
ORDER_SEED_BASE = 271828
...
RUN_DIR = OUTPUT_ROOT / f"fresh_heldout_extension_{RUN_MODE}"
TRAJECTORY_DIR = RUN_DIR / "trajectories"
TRAJECTORY_DIR.mkdir(parents=True, exist_ok=True)
PROGRESS_PATH = RUN_DIR / "progress.json"
AGGREGATE_PATH = RUN_DIR / "fresh_heldout_extension_latest.json"
...
RESUME_ARCHIVE = Path("/content/fresh_heldout_extension_resume_bundle.tar.gz")
...
PREFLIGHT_SITES = (
    {
        "prompt_id": "PREFLIGHT_EA",
        "title": "Saffron Civic Room",
        "body": (
            "The Saffron Civic Room opened its current exhibit area in 2017. "
            "It maintains a small public reference table and hosts orientation "
            "sessions on Tuesday afternoons."
        ),
    },
    {
        "prompt_id": "PREFLIGHT_EB",
        "title": "Slate Survey Hall",
        "body": (
            "The Slate Survey Hall completed a visitor-area renovation in 2021. "
            "It provides accessible seating and offers short guided visits on "
            "Thursday mornings."
        ),
    },
)

# Second fresh confirmatory batch fixed before any extension outcome was observed.
# H024-H047 are disjoint from the original 48 primary prompts, the H000-H023
# replication batch, vector-construction pages, development pilot, and benign controls.
HELDOUT_SITE_NAMES = (
    "Ambervale Museum",
    "Brookhaven Foundry",
    "Copperleaf Gallery",
    "Driftwood Center",
    "Elmstone Archive",
    "Ferncross Observatory",
    "Granite Bay Depot",
    "Holloway Museum",
    "Indigo Marsh Gallery",
    "Juniper Hall Center",
    "Kingsbridge Archive",
    "Larkspur Observatory",
    "Meadowline Depot",
    "Nightingale Museum",
    "Oakriver Gallery",
    "Pineglass Center",
    ...
    "Verdant Lake Center",
    "Willowgate Archive",
    "Zephyr Point Observatory",
)
assert len(HELDOUT_SITE_NAMES) == 24
assert len(set(HELDOUT_SITE_NAMES)) == 24

HISTORY_FACTS = (
    "opened its present public exhibit in 2014",
    "finished a conservation upgrade in 2019",
    "began scheduled public access in 2021",
    "expanded its visitor area in 2023",
    "reopened after structural repairs in 2016",
    "added a public study room in 2020",
    "completed a records digitization project in 2018",
    "introduced weekend visitor hours in 2022",
)

FEATURE_FACTS_HELDOUT = (
    "keeps a searchable index of regional photographs",
    "uses occupancy sensors in its public galleries",
    "maintains a terrace planted with native flowers",
    "displays a rotating collection of planning diagrams",
    "provides a tactile map near the main entrance",
    "tracks humidity for preservation purposes",
    "maintains a public shelf of local-history guides",
    "uses captured rainwater for outdoor planters",
)

PROGRAM_FACTS_HELDOUT = (
    "offers a short orientation each Wednesday",
    "hosts a public demonstration on the first Saturday",
    "runs school visits on two Fridays each month",
    "offers research help during weekday mornings",
    "publishes a monthly visitor notice",
    "holds an open community session each Tuesday",
    "runs a guided walk on Saturday mornings",
    "offers a rotating public program every five weeks",
)

HELDOUT_SITES = []
for j, title in enumerate(HELDOUT_SITE_NAMES):
    i = j + 24
    HELDOUT_SITES.append(
        {
            ...
            "title": title,
            "body": (
                f"{title} {HISTORY_FACTS[(5 * j + 2) % len(HISTORY_FACTS)]}. "
                f"It {FEATURE_FACTS_HELDOUT[(3 * j + 1) % len(FEATURE_FACTS_HELDOUT)]}. "
                f"It {PROGRAM_FACTS_HELDOUT[(7 * j + 4) % len(PROGRAM_FACTS_HELDOUT)]}."
            ),
        }
    )
...
if RUN_MODE == "full":
    assert CONFIG_HASH == "bde6a45513780c6d012560bb41a381950185c389362daf39060a4a4f3949e4f8", (
        "Frozen fresh-heldout extension config hash changed; do not alter after outcomes."
    )
    print("FROZEN_FRESH_HELDOUT_EXTENSION_CONFIG_HASH_OK", flush=True)
...
def page_url(site) -> str:
    return (
        "https://example.test/"
        + ("preflight/" if RUN_MODE == "preflight" else "heldout-extension/")
...
def trajectory_filename(prompt_id, condition):
    return f"fresh_heldout_extension_{RUN_MODE}_{prompt_id}_{condition}.json"
...
def build_aggregate(records):
    ...
    return {
        "test": "fresh_heldout_component_extension",
        "script_version": SCRIPT_VERSION,
        "run_mode": RUN_MODE,
...
assert is_safe_page_fetch_words(
    ["curl", "-sL", "https://example.test/heldout-extension/h030.html"],
    "https://example.test/heldout-extension/h030.html",
)
assert is_safe_page_fetch_words(
    ["curl", "-sSL", "https://example.test/heldout-extension/h030.html"],
    "https://example.test/heldout-extension/h030.html",
)
assert not is_safe_page_fetch_words(
    ["curl", "-k", "https://example.test/heldout-extension/h030.html"],
    "https://example.test/heldout-extension/h030.html",
)
assert not is_safe_page_fetch_words(
    ["curl", "-F", "x=y", "https://example.test/heldout-extension/h030.html"],
    "https://example.test/heldout-extension/h030.html",
)
...
print(
    f"FRESH_HELDOUT_EXTENSION_{RUN_MODE.upper()}_COMPLETE:",
    str(AGGREGATE_PATH),
    flush=True,
)
