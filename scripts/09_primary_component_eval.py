"""Overnight-safe primary causal component evaluation for Role Causal Lever.

Modes
-----
RUN_MODE = "full"  # preflight passed; frozen full evaluation
    Runs two development prompts that are NOT part of the held-out evaluation.
    Use this first while awake.

    Runs the frozen 48-prompt held-out evaluation. Each prompt is evaluated under
    baseline, full, probe-aligned, and residual steering.
...
SCRIPT_VERSION = "0.4.0-primary-component"
RESUME_PATCH = (
    "2026-09-10: accept safe combined curl fetch flags such as -sL; "
    "retry only stale pre-fetch fixture-miss checkpoints; record missing "
    "pre-divergence histories without aborting the overnight batch"
)
...
RUN_DIR = OUTPUT_ROOT / f"primary_component_{RUN_MODE}"
TRAJECTORY_DIR = RUN_DIR / "trajectories"
TRAJECTORY_DIR.mkdir(parents=True, exist_ok=True)
PROGRESS_PATH = RUN_DIR / "progress.json"
AGGREGATE_PATH = RUN_DIR / "primary_component_latest.json"
...
RESUME_ARCHIVE = Path("/content/primary_resume_bundle.tar.gz")
...
if RUN_MODE == "full":
    assert CONFIG_HASH == "747cc2743a5cf79d5de545cae8befbf2b083ec7d4fc6fc9a20a82cc432976df4", (
        "Frozen full-run config hash changed; do not resume old checkpoints."
    )
    print("FROZEN_FULL_CONFIG_HASH_OK", flush=True)
...
def trajectory_filename(prompt_id, condition):
    return f"primary_{RUN_MODE}_{prompt_id}_{condition}.json"
...
def build_aggregate(records):
    ...
    return {
        "test": "primary_component_causal_evaluation",
        "script_version": SCRIPT_VERSION,
        "run_mode": RUN_MODE,
...
print(
    f"PRIMARY_COMPONENT_{RUN_MODE.upper()}_COMPLETE:",
    str(AGGREGATE_PATH),
    flush=True,
)
